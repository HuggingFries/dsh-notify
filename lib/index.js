...
/**
 * Service worker served at /dsh-notify/sw.js (scope /dsh-notify/). It only
 * handles notification clicks: focus an existing dsh web tab and ask it to
 * open the notice's session; if no tab is alive, open the app page with a
 * ?dsh-notify=<sessionId> deep link. No fetch interception, so the app page
 * is never routed through this worker.
 */
const SW_SOURCE = [
  '/* dsh-notify service worker v0.2.6 */',
  ...
  '    if (target) {',
  '      await target.focus()',
  '      target.postMessage({ type: "dsh-notify:open-session", sessionId })',
  '      return',
  '    }',
  '    const url = origin + (sessionId ? "/?dsh-notify=" + encodeURIComponent(sessionId) : "/")',
 ...
 * Accepts both flat calls ({ question, header, options }) and the nested
 * multi-question form ({ questions: [{ id, question, header, options, ... }] })
 * used by the product ask tool; up to three question texts are joined.
 */
...
export function apply(ctx) {
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler(req, res) {
      ...
      // A fresh page (no Last-Event-ID / since) must not replay ancient buffered
      // notices: only the last 15 minutes are replayed on first connect.
      const replayLimitMs = 15 * 60 * 1000
      const now = Date.now()
      for (const notice of notices) {
        if (notice.seq <= since) continue
        if (isFresh && now - notice.time > replayLimitMs) continue
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler(req, res) {
      ...
      req.on('end', () => {
        if (aborted) return
        writeLog(`client ${clip(body, 1024)}`)
  ...
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-notify/sw.js',
    handler(req, res) {
      sendText(res, 200, SW_SOURCE, 'application/javascript; charset=utf-8')
    },
  }))
...
function sendJson(res, code, payload) {
  sendText(res, code, JSON.stringify(payload), 'application/json; charset=utf-8')
}
