...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Baseline module-table seed of the dsh web shell (platform static UI
		// libraries): defineStore moved here after @deepseek-ai/dsh-client-runtime
		// was removed in dsh 0.1.2-rc.1.
		const _client_store = require("@deepseek-ai/dsh-client-store");
		...
		/**
		 * Register the /dsh-notify/sw.js service worker (once). Notifications
		 * shown through the registration are click-handled by the worker, which
		 * focuses an existing dsh web tab and asks it to open the notice's
		 * session — this avoids the browser's default "open a new tab" behavior
		 * for page-created notifications.
		 */
		function registerSw() {
			if (swRegistration !== null) return swRegistration;
			try {
				...
				swRegistration = navigator.serviceWorker
					.register("/dsh-notify/sw.js", { scope: "/dsh-notify/" })
					.catch((error) => {
						console.warn("[dsh-notify] service worker registration failed", error);
						return null;
					});
			} catch (error) {
				swRegistration = Promise.resolve(null);
		...
		async function showDesktop(ctx, notice) {
			...
			// Preferred: service-worker notification — clicks are handled by the
			// worker (focus existing tab + open session) even when this page is
			// backgrounded or was closed.
			try {
				const reg = await registerSw();
				if (reg && typeof reg.showNotification === "function") {
					await reg.showNotification(text.title, Object.assign({}, options, {
						data: { sessionId: notice.sessionId || "" },
					}));
					return true;
				}
			} catch (error) {
				console.warn("[dsh-notify] service worker notification failed", error);
			}
			// Fallback: page-created notification (click handler runs only while
			// this page is alive).
			try {
				const notification = new Notification(text.title, options);
		 ...
		 */
		async function handleNotice(ctx, notice) {
			...
			if (permission === "granted") {
				shown = await showDesktop(ctx, notice);
			}
		...
		function createNotifyStore() {
			return _client_store.defineStore({
		...
		function apply(ctx) {
			...
			// Service worker: notifications shown through it are click-handled by
			// the worker (focus existing tab / deep link) instead of the browser
			// opening a new blank tab. Warm up registration on boot.
			registerSw().catch(() => {});
			try {
				if (typeof navigator !== "undefined" && "serviceWorker" in navigator) {
					navigator.serviceWorker.addEventListener("message", (event) => {
						const data = event.data;
						if (data && data.type === "dsh-notify:open-session") {
							jumpTo(ctx, typeof data.sessionId === "string" ? data.sessionId : "");
						}
					});
				}
			} catch { /* ignore */ }
			// Deep link (?dsh-notify=<sessionId>): used when the service worker
			// had to open a fresh app page for a clicked notification.
			try {
				const match = /[?&]dsh-notify=([^&]+)/.exec(window.location.search);
				if (match) jumpTo(ctx, decodeURIComponent(match[1]));
			} catch { /* ignore */ }
