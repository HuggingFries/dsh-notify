...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function handleNotice(ctx, notice) {
			...
			if (seq <= readShown()) return; // already handled (dedupe across tabs + replay safety)
			...
			markShown(seq); // marked only once something was actually displayed
			logClient({ m: shown ? "os" : "toast", seq, kind });
		}
